...
@if (($variant ?? 'side') === 'chips')
    <div class="no-scrollbar flex gap-2 overflow-x-auto lg:hidden">
        <a class="inline-flex h-9 shrink-0 items-center gap-1.5 rounded-full bg-[#E8ECF3] px-3.5 text-[13px] font-bold text-[#0E1117]" href="{{ route('site.sport', ['when' => request('when', 'all'), 'market' => request('market', 'result')]) }}">
...
@else
    <div class="rounded-xl bg-[#151A23] p-2">
        ...
        <a class="flex items-center justify-between rounded-lg bg-[#1E2533] px-3 py-2.5 text-sm font-semibold text-white" href="{{ route('site.sport', ['when' => request('when', 'all'), 'market' => request('market', 'result')]) }}">
            <span>{{ __('sport.sports.football') }}</span>
    ...
    <div class="rounded-xl bg-[#151A23] p-2">
        ...
        @foreach ($leagues as $league)
            <a class="flex items-center justify-between px-3 py-2 text-[13px] {{ (string) request('league') === (string) $league->id ? 'font-bold text-white' : 'text-[#C9D1DD]' }}" href="{{ route('site.sport', ['league' => $league->id, 'when' => request('when', 'all'), 'market' => request('market', 'result'), 'q' => request('q')]) }}">
